...
### 2026-07-20 (v0.4) — 검색·커뮤니티·승인 플로우·SSO·튜토리얼 고도화
- **검색 서비스**: 검색 필터 바(난이도·평점·학습시간·수료증·가격·태그) + 추천 검색어(관리자 등록 → 헤더 제안·강의목록 칩)
- **커뮤니티 Q&A**: 강의별 질문&답변, 관리자(지식공유자) 답변 뱃지
- **수강신청 승인 플로우**: 바로 수강신청 → 승인중 → 승인/불가 알림 → 내 학습 자동 반영 (+ 관리자 승인 패널)
- **개별학습(사외교육·자격증·학위)**: 사전신청 → 승인 → 결과보고 → 이수 인정 (+ 관리자 이력관리 패널)
- **Microsoft Teams(Entra ID) SSO**: OAuth2 Authorization Code Flow, 이메일·이름 자동완성 회원가입, 데모 모드(`TEAMS_SSO=1`로 실제 전환)
- **온보딩 튜토리얼**: 기획서(화면설명 수정본) 실제 화면 캡처로 6단계 교체 · 회원가입 필수화 · 건너뛰기 동선 정리
- **홈 추천 배너/역량진단 UI**: 강의 썸네일 모자이크(광고 배너형) + "추천강좌" 헤더 · 진단 페이지 건너뛰기→홈
- **서버 안정화**: 대시보드/카탈로그 캐시 오염 방지(실패 시 리셋) + `inflearn()` 자동 재시도(3회) → 간헐 500 해결

> 상세 내역: [`docs/버전아카이빙/2026-07-20_개발내역.md`](docs/버전아카이빙/2026-07-20_개발내역.md)
